...
const ClinicaGeralPrevencao = () => {
  ...
  return (
    <TreatmentPageTemplate
      ...
      introduction="A base para um sorriso saudável e duradouro começa com a prevenção científica e os cuidados de rotina baseados em evidências consolidadas. Na clínica da Dra. Carla Christoph em Ipanema, oferecemos um acompanhamento completo em clínica geral, focado em manter sua saúde bucal através de protocolos avançados que podem prevenir até 90% dos problemas bucais. Com mais de 20 anos de experiência clínica e especialização em Prótese Dental e Implantodontia, nossa abordagem integral da saúde bucal combina prevenção baseada em evidências científicas, tecnologia de ponta e diagnóstico precoce de condições como cáries em estágios reversíveis, problemas gengivais e halitose de origem bucal, oferecendo cuidados personalizados que podem reduzir em até 90% a necessidade de tratamentos complexos futuros."
      sections={[
        {
        ...
        },
        {
          ...
          content: "O diagnóstico precoce representa a essência da odontologia preventiva moderna, utilizando tecnologias e protocolos científicos que identificam alterações em estágios reversíveis ou facilmente tratáveis. **Detecção de Lesões Cariosas Incipientes:** Métodos de fluorescência quantitativa (DIAGNOdent) identificam alterações metabólicas bacterianas antes da cavitação visível, permitindo remineralização através de protocolos específicos com flúor, cálcio e fosfato. Estas lesões podem ser completamente revertidas sem necessidade de restaurações. **Identificação de Problemas Gengivais Precoces:** Análise do fluido crevicular, sondagem periodontal computadorizada, e detecção de sangramento marginal identificam alterações inflamatórias antes da perda de inserção ou retração gengital visível. **Diagnóstico de Retração Gengival Incipiente:** A retração gengival em estágios iniciais é detectada através de análise comparativa fotográfica, medições precisas da margem gengival, e avaliação de fatores de risco como trauma de escovação e doenças periodontais. **Análise de Fatores Causais do Mau Hálito:** Investigação sistemática incluindo análise da saburra lingual, teste de fluxo salivar, identificação de cáries ocultas, avaliação de doenças gengivais, e correlação com fatores sistêmicos quando indicado. **Detecção de Dentes Trincados:** A identificação precoce de trincas invisíveis através de transiluminação, teste de mordida específico, e análise de sintomatologia permite tratamento conservador antes da evolução para fraturas que comprometeriam a preservação dental. Esta abordagem diagnóstica transforma o conceito de 'aguardar sintomas aparecerem' em 'prevenir problemas antes que se desenvolvam'.",
          type: "default"
        },
        {
